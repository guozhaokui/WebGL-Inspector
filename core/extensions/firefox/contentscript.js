...
function notifyPresent() {
    if (global["browser"]) {
        browser.runtime.sendMessage({ present: true }).catch(e => {
            console.error(e);
        });
    } else if (window["chrome"]) {
...
function notifyEnabled(present) {
    if (global["browser"]) {
        browser.runtime.sendMessage({ present: present }).catch(e => {
            console.error(e);
        });
    } else if (window["chrome"]) {
...
console.log("inject script");
    ...
    if (elem) {
      ...
      // TODO: This is not true. Google guidelines now even suggest having no body and no head
      if (elem.firstChild) {
